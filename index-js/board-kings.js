...
const url = 'https://mosttechs.com/board-kings-free-rolls/';
const currentDate = getCurrentDate();
const dir = 'links-json';
const filePath = path.join(dir, 'board-kings.json');
const htmlFilePath = path.join('_includes', 'board-kings.html');
...
async function main() {
  try {
    ...
    $('a[href*="d10x.co"], a[href*="boardkings.onelink.me"]').each((index, element) => {
    ...
    combinedLinks.forEach(link => {
      ...
      htmlContent += `    <span>Free Rolls ${formattedDate}</span>\n`; // Custom text with formatted date
